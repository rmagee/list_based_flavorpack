...
class ListBasedRegionSerializer(RegionSerializer):
    ...
    class Meta(object):
        ...
        fields = (
        'id', 'pool', 'created_date', 'modified_date', 'readable_name',
    ...
    def create(self, validated_data):
        ...
        for processing_parameter in processing_parameters:
            models.ProcessingParameters.objects.create(
                list_based_region=region, **processing_parameter)
        return region
    ...
    def update(self, instance, validated_data):
        ...
        posted_ids = set(
            param.get('id', None) for param in processing_parameters)
        existing_params = set(models.ProcessingParameters.objects.filter(
            list_based_region=instance).values_list('id', flat=True))
        ids_for_removal = existing_params - posted_ids
        objects_for_removal = models.ProcessingParameters.objects.filter(
            list_based_region=instance, pk__in=ids_for_removal)
